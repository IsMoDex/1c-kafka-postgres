...
>
> В ТЗ используется историческое имя `docker-compose.yml`. Проект оставляет
> современное стандартное имя `compose.yaml`: Docker Compose автоматически его
> обнаруживает, а содержимое файла реализует требуемый состав окружения.
...
| Компонент | Роль |
|---|---|
| **1С 8.5.1.1302 Community** | Источник справочных данных (собственный HTTP-сервис). Запускается вне Docker. |
...
Приоритетный источник по ТЗ — **реальная 1С:Предприятие** — реализован и
проверен end-to-end на платформе **8.5.1.1302 Community** (Windows + IIS).
Совместимость приложенной конфигурации с 1С 8.3 не заявляется. Версия 8.5 была
выбрана как доступная среда, в которой удалось воспроизводимо автоматизировать
файловую ИБ, собственный HTTP-сервис и публикацию через IIS; архитектура и
JSON-контракт при этом соответствуют допустимому Варианту Б ТЗ. Полная пошаговая
...
Визуальные доказательства реального контура находятся в
[`1c/screenshots/`](1c/screenshots/README.md): интерфейс 1С, метаданные,
публикация IIS, HTTP JSON, Kafka UI и результаты PostgreSQL.
...
Быстрые unit/component тесты изолированы от живой инфраструктуры и используют
...
Интеграционные pytest-тесты проверяют реальный поднятый контур:

```text
реальная 1С → integration-service → Kafka → consumer-service → PostgreSQL
```

Перед запуском в `.env` должны быть `SOURCE_TYPE=onec` и рабочий
`ONEC_BASE_URL`, а compose-контур должен быть поднят:
...
Интеграционный набор проверяет:
- HTTP-контракт реальной 1С и HTTP 400 для невалидного `changed_since`;
- публикацию событий full sync в реальные Kafka topics;
- применение consumer-ом в PostgreSQL и отсутствие FK-сирот;
- идемпотентность повторного full sync;
- incremental update с восстановлением исходного имени после теста;
- soft delete, health consumer-а и отсутствие новых сообщений в DLQ.

Тест soft delete использует уже предназначенного для демонстрации контрагента
с кодом `000005`. Тест изменения временно меняет контрагента `000001` и в блоке
cleanup возвращает его исходное имя.
...
# 1) восстановить эталонные данные: 4 формы + 5 контрагентов
# /seed обновляет известные demo-записи и снимает с них пометку удаления
curl -X POST $B/seed
...
# 4) изменить контрагента прямо в 1С (query-параметры кодируются curl)
curl -G -X POST "$B/touch" \
  --data-urlencode "id=<guid>" \
  --data-urlencode "name=ООО Ромашка (обновлено)"
...
- Для реального источника: **1С:Предприятие 8.5.1.1302 Community** и IIS.
  Совместимость приложенной конфигурации с 1С 8.3 не заявляется; см. `1c/setup.md`.
