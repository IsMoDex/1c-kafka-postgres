...
- **`updated_at` обязателен** — `null` и datetime без timezone отклоняются до
  публикации в Kafka. Значение должно быть RFC3339 и обозначает время изменения
  объекта в источнике, а не время публикации события.
- **Исторические ссылки сохраняются** — soft-deleted форма собственности остаётся
  допустимым справочным значением. Существующий активный контрагент может ссылаться
  на неё, как это допускает 1С при пометке объекта на удаление.
